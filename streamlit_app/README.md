# 올에셋 라인업 빌더 - Streamlit 버전

축구 라인업처럼 주식 포트폴리오를 구성하는 Streamlit 웹앱입니다!

## ✨ 주요 기능

- ⚽ **4-3-3 포메이션**: 실제 축구처럼 11개 포지션에 종목 배치
- 📱 **모바일 최적화**: 세로 레이아웃으로 핸드폰에서 편하게 사용
- 🎯 **간편한 배치**: 포지션을 선택하고 클릭 한 번으로 추가
- 📊 **실시간 데이터**: yfinance로 기업명과 수익률 조회
- 🎨 **성과별 색상**: 수익률에 따라 자동으로 색상 변경 (빨강-파랑 그라데이션)
- 📅 **다양한 기간**: 1주일, 1개월, 6개월, 1년
- 🏴 **거래소 국기**: 각 티커의 거래소 국기 표시
- 🇰🇷 **한국어 인터페이스**: 완전 한국어 지원
- 📷 **스크린샷 기능**: 제목이 포함된 고화질 이미지로 다운로드

## 🚀 로컬 실행 방법

### 1. 의존성 설치

```bash
cd streamlit_app
pip install -r requirements.txt
```

### 2. 앱 실행

```bash
streamlit run app.py
```

브라우저가 자동으로 열립니다! (보통 http://localhost:8501)

## 📱 사용 방법

### PC에서 사용
1. **좌측 사이드바**에서 티커 심볼 입력 (예: AAPL)
2. **포지션 선택** (골키퍼, 수비수, 미드필더, 공격수)
3. **"선수 추가"** 버튼 클릭
4. 최대 11명까지 추가 가능
5. 상단에서 수익률 기간 변경 가능
6. **스크린샷**: 축구장 위에 마우스를 올리면 📷 다운로드 버튼이 나타남

### 모바일에서 사용
1. 사이드바는 왼쪽 상단 화살표로 열고 닫기
2. 세로로 긴 축구장이 화면에 딱 맞게 표시됨
3. 터치로 쉽게 티커 입력 및 포지션 선택
4. 버튼이 전체 너비로 표시되어 터치하기 쉬움
5. **스크린샷**: 축구장을 탭하면 메뉴가 나타나고, 카메라 아이콘을 탭해서 저장

### 📷 스크린샷 기능
- 축구장 위에 마우스를 올리면 우측 상단에 툴바가 나타남
- 📷 카메라 아이콘을 클릭하면 PNG 이미지로 저장됨
- 저장되는 이미지에는 "올에셋 라인업 빌더" 제목이 포함됨
- 고해상도 (3배 스케일)로 저장되어 선명함
- 파일명: `올에셋_라인업.png`

## 🎮 포지션 설명

**4-3-3 포메이션** (위에서 아래로)

### 공격진 (FW) - 상단
- 🔴 오른쪽 공격수 (FW1)
- 🔴 중앙 공격수 (FW2)
- 🔴 왼쪽 공격수 (FW3)

### 미드필더 (MF) - 중앙
- 🟡 오른쪽 미드필더 (MF1)
- 🟡 중앙 미드필더 (MF2)
- 🟡 왼쪽 미드필더 (MF3)

### 수비진 (DF) - 중하단
- 🔵 오른쪽 풀백 (DF1)
- 🔵 중앙 수비수 1 (DF2)
- 🔵 중앙 수비수 2 (DF3)
- 🔵 왼쪽 풀백 (DF4)

### 골키퍼 (GK) - 최하단
- 🟢 골키퍼 (GK)

**참고:** 골키퍼가 화면 하단에 위치하며, 공격 방향은 위쪽입니다. (일반적인 라인업 빌더 형식)

## 🌐 배포하기 (3분!)

### Streamlit Community Cloud (완전 무료!)

1. **Streamlit Cloud 접속**
   - https://streamlit.io/cloud 접속
   - GitHub으로 로그인

2. **앱 배포**
   - "New app" 클릭
   - **Repository**: 이 저장소 선택
   - **Branch**: `claude/stock-lineup-visualizer-011CUPf4kzhYdQ39asps1LBh`
   - **Main file path**: `streamlit_app/app.py`
   - "Deploy!" 클릭

3. **완료!**
   - 2-3분 후 배포 완료
   - 제공되는 URL로 접속
   - 예: `https://your-app.streamlit.app`

**환경 변수 설정 불필요!**
**별도 백엔드 배포 불필요!**
**설정 파일 불필요!**

### 자동 업데이트

GitHub에 푸시하면 자동으로 재배포됩니다:

```bash
git add .
git commit -m "기능 업데이트"
git push
```

→ Streamlit Cloud가 자동으로 감지하고 재배포!

## 🎨 색상 코드 (성과별)

**수익률에 따른 그라데이션 시스템**

### 🔴 플러스 수익률 (빨간색)
- **0%**: 매우 연한 빨강 (#FFB3B3)
- **10%**: 중간 빨강
- **20%**: 밝은 빨강
- **30% 이상**: 진한 빨강 (#CC0000)

수익률이 높을수록 더 진한 빨간색으로 표시됩니다.

### 🔵 마이너스 수익률 (파란색)
- **0%**: 매우 연한 파랑 (#B3D9FF)
- **-10%**: 중간 파랑
- **-20%**: 밝은 파랑
- **-30% 이하**: 진한 파랑 (#0066CC)

수익률이 낮을수록 더 진한 파란색으로 표시됩니다.

### 회색
- **데이터 없음**: 회색 (#A0AEC0)

## 💡 추천 티커

### 기술주 (테크)
- **AAPL** - Apple (애플)
- **GOOGL** - Google (구글)
- **MSFT** - Microsoft (마이크로소프트)
- **TSLA** - Tesla (테슬라)
- **NVDA** - NVIDIA (엔비디아)
- **META** - Meta/Facebook (메타)
- **AMZN** - Amazon (아마존)

### 금융주
- **JPM** - JPMorgan Chase
- **BAC** - Bank of America
- **GS** - Goldman Sachs
- **V** - Visa (비자)
- **MA** - Mastercard (마스터카드)

### 소비재
- **WMT** - Walmart (월마트)
- **KO** - Coca-Cola (코카콜라)
- **PEP** - PepsiCo (펩시)
- **MCD** - McDonald's (맥도날드)
- **NKE** - Nike (나이키)

### 헬스케어
- **JNJ** - Johnson & Johnson
- **PFE** - Pfizer (화이자)
- **UNH** - UnitedHealth
- **ABBV** - AbbVie

## 🔄 Flask/React 버전과의 차이점

| 기능 | Flask/React | Streamlit (이 버전) |
|------|-------------|---------------------|
| **티커 배치** | 드래그앤드롭 | 포지션 선택 방식 |
| **배포 난이도** | 중간 (8분) | 매우 쉬움 (3분) |
| **모바일 최적화** | 반응형 | 세로 레이아웃 최적화 |
| **언어** | 영어 | 한국어 |
| **포지션** | 자유 배치 | 4-3-3 포메이션 |
| **실시간 데이터** | ✅ | ✅ |
| **수익률 표시** | ✅ | ✅ |
| **국기 표시** | ✅ | ✅ |
| **무료 호스팅** | ✅ | ✅ |

## 📱 모바일 최적화 특징

- ✅ 세로로 긴 축구장 (핸드폰 화면에 최적화)
- ✅ 터치 친화적인 버튼 크기
- ✅ 사이드바 토글 (메뉴가 거슬리지 않음)
- ✅ 반응형 폰트 크기
- ✅ 한 손으로 조작 가능
- ✅ 스크롤 최소화

## 🛠️ 문제 해결

### 티커 데이터를 불러올 수 없음

**원인:** 유효하지 않은 티커 심볼이거나 네트워크 문제

**해결:**
- 올바른 티커 심볼인지 확인 (대문자로 입력)
- 인터넷 연결 확인
- 다른 티커로 시도 (예: AAPL)

### 포지션이 선택되지 않음

**원인:** 해당 포지션이 이미 차 있음

**해결:**
- 다른 포지션 선택
- 또는 기존 선수를 삭제하고 다시 추가

### Streamlit Cloud 배포 실패

**원인:** Python 버전 문제

**해결:**
- `.python-version` 파일이 있는지 확인 (Python 3.11 지정)
- requirements.txt가 정확한지 확인

## 📊 기술 스택

- **Streamlit** - 웹 프레임워크
- **Plotly** - 인터랙티브 그래픽 (축구장 시각화)
- **yfinance** - 주식 데이터 API
- **Pandas** - 데이터 처리
- **Python 3.11** - 안정적인 실행 환경

## 📁 프로젝트 구조

```
streamlit_app/
├── app.py               # 메인 애플리케이션
├── requirements.txt     # Python 의존성
├── .python-version      # Python 버전 (3.11)
├── .streamlit/
│   └── config.toml     # Streamlit 설정
└── README.md           # 이 파일
```

## 🆕 업데이트 내역

### v2.0 (최신)
- ✅ 클릭 문제 해결 → 포지션 선택 방식으로 변경
- ✅ 전체 한국어 변환
- ✅ 모바일 최적화 (세로 레이아웃)
- ✅ 4-3-3 포메이션 도입
- ✅ 반응형 디자인
- ✅ UX 개선

### v1.0
- 초기 버전 (클릭 방식, 영어)

## 🎯 향후 개선 계획

- [ ] 다양한 포메이션 (4-4-2, 3-5-2 등)
- [ ] 라인업 저장/불러오기
- [ ] 이미지로 내보내기 (공유용)
- [ ] 섹터/산업 정보 추가
- [ ] 여러 라인업 비교 기능
- [ ] 히스토리 성과 추적
- [ ] 다크 모드

## 📜 라이센스

MIT License - 자유롭게 사용하세요!

## 💬 피드백

- GitHub Issues로 버그 제보
- 기능 제안 환영
- Pull Request 환영

## 🙏 도움말

- Streamlit 문서: https://docs.streamlit.io/
- yfinance 문서: https://pypi.org/project/yfinance/
- Plotly 문서: https://plotly.com/python/

---

**만든 사람:** Claude Code와 함께
**목적:** 재미있게 주식 포트폴리오 관리하기
**라이센스:** MIT

⚽ + 📈 = 💰

즐겁게 사용하세요! 🚀
